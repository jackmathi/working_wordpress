...
class SU_Titles extends SU_Module {
	...
	function change_title_tag($head) {
		...
		$classArray = get_body_class();		
		if ( in_array('woocommerce-lost-password', $classArray ) ) {
			$title = 'VOGT | Forgot Password';
		}
		if ( in_array('woocommerce-order-received', $classArray ) ) {
			$title = 'VOGT | Order recieved';
		}
		if ( in_array('woocommerce-edit-account', $classArray ) ) {
			$title = 'VOGT | Account Information';
		}
		if ( in_array('woocommerce-view-order', $classArray ) ) {
			$title = 'VOGT | Order Details';
		}
		if(strpos($page_title,'change-password') !== false){ 
			$title = 'VOGT | Change Password';
		}
		if(strpos($page_title,'billing') !== false){ 
			$title = 'VOGT | Edit Address';
		}
		if(strpos($page_title,'orders') !== false){ 
			$title = 'VOGT | Order History';
		}
