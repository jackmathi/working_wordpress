...
<?php
$ac = isset($_GET['ac']) ? sanitize_text_field($_GET['ac']) : '';
$email = "";
if (isset($_GET['ac']) && $_GET['ac'] == 'email')
{
...
<div class="wrap">
  ...
  <h3><?php _e('Download History (By Email)', 'email-download-link'); ?></h3>
	<?php
	$pagenum = isset( $_GET['pagenum'] ) ? absint( $_GET['pagenum'] ) : 1;
	$limit = 100;
	$offset = ($pagenum - 1) * $limit;
	$total = ed_cls_subscribers::ed_subscribers_count(0, "", "", $email);
	$total = ceil( $total / $limit );
	$myData = array();
	$myData = ed_cls_subscribers::ed_subscribers_view_page_email($offset, $limit, $email);
	?>
      <table width="100%" class="widefat" id="straymanage">
        <thead>
          <tr>
		  	<!--<th class="check-column" scope="col" style="padding: 8px 2px;"><input type="checkbox" /></th>-->
		...
		if(count($myData) > 0)
		{
			...
			{					
				...
				<!--<td align="left"><input type="checkbox" value="<?php //echo $data['ed_email_id']; ?>"></td>-->
				...
				<?php echo $downloads['ed_form_title']; ?></a>
				</td>
				...
				<img src="<?php echo ED_URL; ?>images/delete.gif" alt="Delete" /></a></td>
				</tr>
				<?php
				$i = $i+1;
			} 
