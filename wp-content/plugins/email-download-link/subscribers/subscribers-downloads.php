...
$id = 0;
if (isset($_GET['ac']) && $_GET['ac'] == 'downloads')
{
	$id = isset($_GET['id']) ? sanitize_text_field($_GET['id']) : '';
	ed_cls_security::ed_check_guid($id);
}
...
<div class="wrap">
  ...
  <h3><?php _e('Download History (By Download URL)', 'email-download-link'); ?></h3>
	<?php
	$pagenum = isset( $_GET['pagenum'] ) ? absint( $_GET['pagenum'] ) : 1;
	$limit = 100;
	$offset = ($pagenum - 1) * $limit;
	$total = ed_cls_subscribers::ed_subscribers_count(0, "", $id, "");
	$total = ceil( $total / $limit );
	$myData = array();
	$myData = ed_cls_subscribers::ed_subscribers_view_page($offset, $limit, 0, "", $id);
	?>
